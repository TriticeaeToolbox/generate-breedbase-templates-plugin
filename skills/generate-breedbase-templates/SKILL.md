...
Every trial needs to have the `location` column set.  The format of the location should always be "Town, ST" (where ST is the state or Canadian province abbreviation).  For example, "Fargo, ND" or "Brandon, MB".
...
Most trials will only have one accession per plot.  However, there are occassionally intercropping trials which may have two or more accessions per plot.  In this case, the primary species accession (usually the oat accession in oat/pea intercropping experiments) will be listed in the `accession_name` column.  Any additional accessions (usually the pea accession in oat/pea intercropping experiment) will be listed in the `intercrop_accession_name` column.  If there are three or more accessions in the plot, the `intercrop_accession_name` can contain a comma-separated list of all additional accessions.  The `accession_name` column is always required while the `intercrop_accession_name` column is optional.  In an intercropping experiment, if the primary species accession (such as the oat accession) is not planted and the secondary species accession (such as the pea accession) is planted, you should ask the user how to proceed: they may want to 1) add the secondary species accession to the `accession_name` column or 2) keep the secondary species accession in the `intercrop_accession_name` column and add a dummy primary species accession (such as 'NO_OATS_PLANTED') to the `accession_name` column.
